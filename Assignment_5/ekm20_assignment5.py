...
from eval import computeAccuracy, computePrecisionRecall
from ekm20_assignment3 import predict_simplistic

# TODO: You may need to modify assignment 4 if you just had a main() there.
# recall_precision_accuracy() should take a column as input and return as output 10 floats (numbers)
# representing the metrics.
from ekm20_assignment4 import recall_precision_accuracy()
...
def main(argv):
    ...
    # TODO: See also the next todo which gives an example of a convenient output for recall_precision_accuracy()
    ...
    nb_original = recall_precision_accuracy(data['review'])
    nb_cleaned = recall_precision_accuracy(data['cleaned_review'])
    nb_lowercase = recall_precision_accuracy(data['lowercased'])
    nb_no_stop = recall_precision_accuracy(data['no stopwords'])
    nb_lemmatized = recall_precision_accuracy(data['lemmatized'])
